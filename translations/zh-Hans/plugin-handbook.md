...
只需使用parentPath：` path.parentPath `调用` replaceWith `即可</p>
...
编写转换时，通常需要构建一些要插入的节点进入AST。 如前所述，您可以使用<code>babel-types </code>包中的<a href="#builders">builder方法。</p>
...
你可以看到`object`需要是`Expression`， ` property `需要是` Expression `或` Identifier `取决于成员表达式是` computed `还是不是` computed `， ` computed `只是一个默认为` false `的布尔值。



因此，我们可以通过以下步骤构造一个`MemberExpression`:
...
然而，我们说过`object`需要是`Expression`，那么为什么`Identifier`是有效的呢?



如果我们看看`Identifier`的定义，我们可以看到它有一个`aliases`属性，表明它也是一个表达式。

```js
aliases: ["Expression", "LVal"],
```

所以由于`MemberExpression`是`Expression`的一种类型，我们可以将它设置为另一个`MemberExpression`的`object`:
...
您不太可能记住每个节点类型的构建器方法签名。因此，您应该花一些时间来理解它们是如何从节点定义生成的。

你会发现所有的定义 [在这里](https://github.com/babel/babel/tree/master/packages/babel-types/src/definitions) ，并且你可以看到它们的文档 [在这里](https://github.com/babel/babel/blob/master/doc/ast/spec.md)

* * *

# <a id="toc-best-practices"></a>最佳实践

## <a id="toc-create-helper-builders-and-checkers"></a> 创建Helper Builders和Checkers

将某些检查(如果某个节点是某种类型)提取到它们自己的helper函数中，以及提取特定节点类型的helper非常简单。
...
遍历AST的开销很大，而且很容易意外地超出必要的范围遍历AST。这可能需要成千上万的额外操作。



Babel尽可能地优化了这一点，将访问者合并在一起，以便在一次遍历中完成所有操作。

### <a id="toc-merge-visitors-whenever-possible"></a>及时合并访问者对象

当写访问者对象时，可能很容易用`path.traverse`。在逻辑上需要的多个地方遍历。
...
然而，将它们写成只运行一次的单个访问者要好得多。否则，您将无缘无故地多次遍历同一棵树。
...
它可能也很容易被称为`path.traverse`。查找特定节点类型时，遍历。
...
但是，如果您正在寻找特定的、浅层的节点，那么您可以手动查找所需的节点，而无需执行代价高昂的遍历。
...
但是，每当调用`FunctionDeclaration()`时都会创建一个新的访问者对象。

这可能是昂贵的，因为每次传入一个新的访问者对象时，Babel都会进行一些处理(比如分解包含多种类型的键、执行验证和调整对象结构)。因为Babel在访问者对象上存储标志，表明它已经执行了该处理，所以最好将访问者存储在一个变量中，并每次传递相同的对象。
...
那么我们可以开始写我们的第一个测试：快照。 快照测试允许我们直观地检查我们的babel插件的输出。 我们给它一个输入，告诉它一个快照，并将其保存到一个文件。 我们检查快照到git中。 这允许我们来看看我们什么时候影响了我们任何试用例子测试的输出。 它也给出了使用差异在拉请求的时候。 当然，您可以用任何测试框架来做到这一点，但是要更新一下快照就像`jest -u `一样简单.</p>
